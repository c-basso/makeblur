 ...
 * - Локали **ja, ko, zh, th, hi, bn, ta, te, ml, he** (в URL также `jp` и `cn`, что сопоставляется с ja/zh) не получают предупреждений по 50–60/150–160
...
/** URL path langs that map to BCP-47/script families above (constants use `jp`/`cn`). */
function langForSerpHeuristic(lang) {
...
function useLatinSerpLengthHeuristic(lang) {
  return !NON_LATIN_SERP_LENGTH_HEURISTIC.has(langForSerpHeuristic(lang));
}

/** После сборки текст в `<title>` / meta может содержать `&#39;` и др.; для длины сравниваем с тем, что видит пользователь/SERP. */
...
function extractDocumentTitle(html) {
  ...
  if (!m) return null;
  return decodeBasicHtmlEntities(m[1].replace(/\s+/g, ' ').trim());
}
...
function validateSeoMetaForPage(html, { file, lang }) {
  ...
  if (description === undefined) {
    ...
    const len = decodeBasicHtmlEntities(String(description).trim()).length;
  ...
  return {
    ...
    meta: {
      ...
      descriptionLength:
        description !== undefined ? decodeBasicHtmlEntities(String(description).trim()).length : null
    }
  };
}
